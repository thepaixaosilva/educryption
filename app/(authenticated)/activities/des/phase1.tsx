...
const ActivityContent: React.FC<ActivityContentProps> = ({ initialStages }) => {
  ...
  const [answers, setAnswers] = useState({
    userGrid: Array.from({ length: 4 }, () => Array(12).fill('')), //12 x 4 linhas
  })
  ...
  const boxExpansao =
    [
      ...
      [8, 9, 10, 11, 12, 13, 12, 13, 14, 14, 16, 17],
  ...
  const handleReset = () => {
    ...
    setAnswers({
      userGrid: Array.from({ length: 4 }, () => Array(12).fill('')),
    })
  }
  ...
  const validation = async (): Promise<boolean | string> => {
    ...
    // Verifica se tem 8 valores preenchidos
    const totalFilled = answers.userGrid.flat().filter((val) => val !== '').length
    if (totalFilled !== 8) return 'Faltam valores a serem preenchidos'
  ...
  useEffect(() => {
    const verificarDesbloqueio = async () => {
      ...
      if (!desbloqueadas.includes('2ª Fase DES - Função 1: Expansão')) { //nome da atividade extraída do QR Code
        Alert.alert(
          'Atividade bloqueada',
  ...
  return (
    <Activity.Root>
      <Activity.Header
        title="Fase 1: Expansão"
        subtitle="Expansão de 32 bits para 48 bits"
        description="Nessa fase, utiliza-se uma tabela que define uma permutação mais uma expansão, envolvendo a duplicação de 16 bits do bloco. Para o exercício, vamos trabalhar com os 5 primeiros valores do bloco. Se precisar de mais informações, consulte o material da aula."
        onReset={handleReset}
        hideReset={false}
      >
        <TouchableOpacity
      ...
      <Activity.Content>
        ...
        <Activity.Reward message="Parabéns! Você completou a etapa de expansão do bloco." />
      </Activity.Content>
    </Activity.Root>
  )
}
